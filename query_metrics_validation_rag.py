#!/usr/bin/env python3
"""
Script interactivo para consultar el Knowledge Graph de validación de métricas

Este script consulta el RAG generado durante la validación de ImageDeduplicator metrics:
- Documento: Catalogo_de_Servicios_y_Prestaciones-6 - copia.pdf
- Ubicación: test_environment/output/metrics_validation_2025-10-17_11-54-39/rag_storage
- Métricas guardadas: deduplication_metrics con 1 imagen (0 duplicados)

Características:
1. Cargar el Knowledge Graph de validación
2. Hacer queries interactivas en distintos modos
3. Ver resultados formateados
4. Verificar métricas de deduplicación
"""
...
def print_banner():
    ...
    print("\n" + "="*80)
    print("   METRICS VALIDATION RAG - QUERY INTERFACE")
    print("   Document: Catalogo_de_Servicios_y_Prestaciones-6 - copia.pdf")
    print("="*80)
    print()
...
def print_help():
    ...
    print("  /metrics           - Mostrar métricas de deduplicación")
...
async def show_metrics(rag: RAGAnything):
    ...
    try:
        ...
        for doc_id, status in doc_status_data.items():
            if 'deduplication_metrics' in status:
                metrics = status['deduplication_metrics']
                print(f"\nDocument: {status.get('file_path', 'unknown')}")
                print(f"Doc ID: {doc_id}")
                print("\nMetrics:")
                print(f"  Total Images:          {metrics.get('total_images')}")
                print(f"  Unique Images:         {metrics.get('unique_images')}")
                print(f"  Duplicate Images:      {metrics.get('duplicate_images')}")
                print(f"  Reduction Percentage:  {metrics.get('reduction_percentage')}%")
                print(f"  VLM Calls Saved:       {metrics.get('vlm_calls_saved')}")
            else:
                print(f"\n⚠️  No deduplication metrics found for doc {doc_id}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Query interface for Metrics Validation RAG",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Query the metrics validation RAG (default path)
  python query_metrics_validation_rag.py test_environment/output/metrics_validation_2025-10-17_11-54-39/rag_storage

  # Use /metrics command to see deduplication metrics
  # Use /info to see KG information
  # Use /help for all commands
    ...
    parser.add_argument(
        "working_dir",
        nargs='?',
        default="test_environment/output/metrics_validation_2025-10-17_11-54-39/rag_storage",
        help="Path to rag_storage directory (default: metrics validation RAG)"
    )
    parser.add_argument(
        "--api-key",
